"""
Генератор стульев — headless скрипт с рендером и галереей.

Использование:
    blender --background --python tools/generators/chairs/generate.py -- [опции]

Опции:
    --type TYPE         dining, office, bar, armchair, stool, bench, mixed
    --count N           Количество (по умолчанию 10)
    --output DIR        Папка для результатов (по умолчанию output/chairs)
...
for _m in list(sys.modules.keys()):
    if _m in ('helpers', 'chair_types'):
        del sys.modules[_m]

from chair_types import generate_chair, CHAIR_TYPES
...
def _generate_fn(item_seed, args, rng):
    active_types = list(CHAIR_TYPES.keys()) if args.type == 'mixed' else [args.type]
    subtype = rng.choice(active_types)
    objects = generate_chair(item_seed, subtype=subtype)
    basename = f"chair_{subtype}_{item_seed}"
...
def main():
    args = parse_args(
        description="Chair generator",
        type_choices=list(CHAIR_TYPES.keys()) + ['mixed'],
    )
