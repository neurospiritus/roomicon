"""
Генератор свечей — headless скрипт.

Использование:
    blender --background --python tools/generators/candles/generate.py -- [опции]

Опции:
    --type TYPE         single, candlestick, group, tealight, mixed
    --count N           Количество (по умолчанию 10)
    --output DIR        Папка для результатов (по умолчанию output/candles)
...
for _m in list(sys.modules.keys()):
    if _m in ('helpers', 'candle_types'):
        del sys.modules[_m]

from candle_types import generate_candle, CANDLE_TYPES
...
def _generate_fn(item_seed, args, rng):
    active_types = list(CANDLE_TYPES.keys()) if args.type == 'mixed' else [args.type]
    subtype = rng.choice(active_types)
    objects = generate_candle(item_seed, subtype=subtype)
    basename = f"candle_{subtype}_{item_seed}"
    return objects, {'basename': basename, 'seed': item_seed, 'subtype': subtype}


def main():
    args = parse_args(
        description="Candle generator",
        type_choices=list(CANDLE_TYPES.keys()) + ['mixed'],
    )
    run_batch(
        args, project_dir,
        category='candles',
        ...
        gallery_title='Candle Generator',
