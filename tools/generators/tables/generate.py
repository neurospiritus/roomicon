"""
Генератор столов — headless скрипт с рендером и галереей.

Использование:
    blender --background --python tools/generators/tables/generate.py -- [опции]

Опции:
    --type TYPE         dining, coffee, desk, nightstand, round, mixed
    --count N           Количество вариантов (по умолчанию 10)
    --output DIR        Папка для результатов (по умолчанию output/tables)
...
for _m in list(sys.modules.keys()):
    if _m in ('helpers', 'table_types'):
        del sys.modules[_m]

from table_types import generate_table, TABLE_TYPES
...
def _generate_fn(item_seed, args, rng):
    active_types = list(TABLE_TYPES.keys()) if args.type == 'mixed' else [args.type]
    subtype = rng.choice(active_types)
    objects = generate_table(item_seed, subtype=subtype)
    basename = f"table_{subtype}_{item_seed}"
...
def main():
    args = parse_args(
        ...
        type_choices=list(TABLE_TYPES.keys()) + ['mixed'],
    )
